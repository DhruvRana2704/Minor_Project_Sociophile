...
function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchFollower = async () => {
      try {
        const response = await fetch('http://localhost:5000/followers/getfollowers', {
          method: 'GET',
          credentials: 'include'
        });
        const data = await response.json();
    ...
    <div className="instagram-home min-vh-100 d-flex flex-column align-items-center justify-content-center"
      ...
      <div className="container-fluid py-5 d-flex flex-column align-items-center justify-content-center"
        ...
        <div className="card p-4 mb-4 text-center instagram-card" style={{ maxWidth: 400, width: '100%', alignItems: "center" }}>
          ...
          <img src={`http://localhost:5000${user.avatar}`} style={{ objectFit: 'cover', height: '200px', width: '200px' }} alt="avatar" className="rounded-circle mb-3" width="100" height="100" />
                  ...
                  <ul className="list-unstyled mb-0">
                    {(showList === 'followers' ? followers : following).map((item, idx) => (
                      <li style={{display:'flex',alignItems:'center'}} key={idx} className="py-2 border-bottom">
                        <img src={showList === 'followers' ? `http://localhost:5000${item.followerId.avatar}` : `http://localhost:5000${item.followingId.avatar}`} width='40px' height='40px' alt="" style={{objectFit:'cover', borderRadius: '50%', marginRight:'10px'}}/>
