...
function Explore() {
  ...
  const handleFollow = async (username) => {
    const response = await fetch('http://localhost:5000/followers/create_follower', {
      method: 'POST',
  ...
  const handleUnfollow = async (username) => {
    const response = await fetch('http://localhost:5000/followers/remove_follower', {
      method: 'POST',
      body: JSON.stringify({ username }),
      credentials: 'include',
      headers: {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const followingRes = await fetch('http://localhost:5000/followers/getfollowers', {
        method: 'GET',
        credentials: 'include'
      });
          ...
          {filteredUsers.map((user) => (
            <div key={user.username} className="list-group-item d-flex align-items-center justify-content-between instagram-card mb-3 p-3">
              <div className="d-flex align-items-center gap-3">
                <img src={`http://localhost:5000${user.avatar}`} style={{ objectFit: 'cover' }} alt={user.name} className="rounded-circle" width="48" height="48" />
                <div>
